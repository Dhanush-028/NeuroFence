...
class ScanWorker(QThread):
    """
    Runs the anomaly detector on a background thread so the app stays
    responsive during a scan (it can take a few minutes). Without this,
    the window would show "Not Responding" the whole time.
    """
    ...
    def run(self):
        try:
            ...
            flagged = run_detection(self.model_path)
            self.finished_signal.emit(("ok", flagged))
...
class NeuroFenceWindow(QWidget):
    ...
    def _build_ui(self):
        ...
        tabs.addTab(self._build_scan_tab(), "Backdoor Scan")
